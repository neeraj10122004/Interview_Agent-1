...
class BedrockChat extends chat_models_1.BaseChatModel {
    ...
    async _signedFetch(messages, options, fields) {
        ...
        const inputBody = this.usesMessagesApi
            ? bedrock_js_1.BedrockLLMInputOutputAdapter.prepareMessagesInput(provider, messages, this.maxTokens, this.temperature, options.stop ?? this.stopSequences, this.modelKwargs, this.guardrailConfig)
            : bedrock_js_1.BedrockLLMInputOutputAdapter.prepareInput(provider, convertMessagesToPromptAnthropic(messages), this.maxTokens, this.temperature, options.stop ?? this.stopSequences, this.modelKwargs, fields.bedrockMethod, this.guardrailConfig);
